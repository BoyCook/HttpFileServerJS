...
HttpServer.prototype.start = function (fn) {
    ...
    server = http.createServer(function (req, res) {
        ...
        var path = context.checkPath(inboundPath);
        ...
        switch (req.method) {
            case "GET":
                ...
                break;
            case "DELETE":
                context.processDELETE(res, inboundPath);
                break;
            case "PUT":
                context.processPUT(req, res, inboundPath, path);
                break;
            default:
                res.writeHead(405);
...
HttpServer.prototype.checkPath = function(pathStr) {
    var path = undefined;
    if (this.routes) {
        path = this.routes.match(pathStr);
    }
    return path;
};
...
HttpServer.prototype.processPUT = function (req, res, inboundPath, path) {
    var context = this;
    var data = "";
    ...
    req.on("end", function () {
        ...
        fs.save(context.baseDir + inboundPath, data);
        res.writeHead(201);
        res.end(data);
    });
};
